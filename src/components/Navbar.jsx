import { useEffect, useState } from "react";
  ...
  return (
    <nav className={`flex items-center justify-between w-full h-20 bg-white px-[130px] sticky top-0 z-50 transition-shadow duration-500 ${scrolled ? "shadow-md" : ""}`}>
      ...
      <div className="flex items-center p-4 gap-8 font-semibold text-gray-600">
        <a href="#" className={`py-1 hover:text-sky-700 text-sm transition-all ${selected === 1 ? "text-sky-700 font-bold border-b-2 border-sky-700" : ""}`} onClick={() => setSelected(1)}>
          Accueil
        </a>
        <a href="#" className={`py-1 hover:text-sky-700 text-sm transition-all ${selected === 2 ? "text-sky-700 font-bold border-b-2 border-sky-700" : ""}`} onClick={() => setSelected(2)}>
          L{`'`}entreprise
        </a>
        <a href="#" className={`py-1 hover:text-sky-700 text-sm transition-all ${selected === 3 ? "text-sky-700 font-bold border-b-2 border-sky-700" : ""}`} onClick={() => setSelected(3)}>
          Services
        </a>
        <a href="#" className={`py-1 hover:text-sky-700 text-sm transition-all ${selected === 4 ? "text-sky-700 font-bold border-b-2 border-sky-700" : ""}`} onClick={() => setSelected(4)}>
          Équipements
        </a>
        <a href="#" className={`py-1 hover:text-sky-700 text-sm transition-all ${selected === 5 ? "text-sky-700 font-bold border-b-2 border-sky-700" : ""}`} onClick={() => setSelected(5)}>
          Contact
        </a>
